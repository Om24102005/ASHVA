...
function viewGarage(app){
  const u=app.s.user;
  const name=(u&&u.displayName)||'Rider';
  ...
  return `<div style="padding:64px 0 0">
    <div style="padding:0 24px 22px;display:flex;align-items:center;gap:16px">
      <div style="width:72px;height:72px;border-radius:50%;background:linear-gradient(140deg,${C.ember},${C.amber});display:flex;align-items:center;justify-content:center;font-family:${F.s};font-size:32px;color:#fff">${initial}</div>
      <div><div style="font-family:${F.s};font-size:28px;line-height:1.05">${name}</div>
